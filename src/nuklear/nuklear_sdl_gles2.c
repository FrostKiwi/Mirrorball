...
nk_sdl_render(enum nk_anti_aliasing AA, int max_vertex_buffer, int max_element_buffer)
{
	...
	SDL_GetWindowSize(sdl.win, &width_scale, &height_scale);
	SDL_GL_GetDrawableSize(sdl.win, &width, &height);
	...
	sdl.scale.x = (float)display_width / (float)width_scale;
	sdl.scale.y = (float)display_height / (float)height_scale;
...
nk_sdl_handle_event(SDL_Event *evt)
{
	...
	case SDL_MOUSEBUTTONDOWN:
	{
		...
		const int x = evt->button.x * sdl.scale.x, y = evt->button.y * sdl.scale.y;
	...
	case SDL_MOUSEMOTION:
		if (ctx->input.mouse.grabbed)
		{
			...
			nk_input_motion(ctx, evt->motion.x * sdl.scale.x,
							evt->motion.y * sdl.scale.y);
		return 1;
